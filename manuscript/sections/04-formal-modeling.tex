...
are composed and admitted for one abstract \batchreceive execution. This
choice removes cryptographic and application-layer detail that is orthogonal
to the question in Section~3: which identity coordinate is constrained at the
batch-admission boundary? The model therefore retains explicit party,
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\linewidth}{@{}
    ...
    Message-level restriction & Admit if $m_1\neq m_2$; provide an
      equal-message rejection branch & Coordinate-surrogate control \\
...
sender-session or sender-occurrence coordinate, and $m$ is a fresh message
coordinate. Party creation records a persistent principal fact. A sender
...
property is deferred to Section~5. The model is not presented as ``broken
K-Waay'' and does not assert how the real protocol or a deployed caller handles
such inputs.
...
\emph{reachability} obligations ask whether representative admitted or
rejected behaviors, including normal distinct-party behavior and candidate
repetition patterns, have a trace. Such obligations are also needed to rule
out vacuous safety caused by admitting no batch. Second,
...
batch and receiver context; it is not equivalent to party uniqueness.
