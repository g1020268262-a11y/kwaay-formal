...
\cite[Sec.~5.1]{collins2024kwaay}. Thus, the distinct-party condition is prior
...
properties \cite{unger2015sok}. General compositional-verification frameworks
...
\cite{sethi2019misbinding}. The TLS/SDP unknown key-share analysis similarly
...
in Section~5, the message-level variant can maintain a one-to-one relation
between the modeled sender and receiver occurrences while still admitting two
different messages from the same party. This does not show that existing
...
Within the verified literature corpus and search scope used for this paper, we
found no prior analysis that isolates \kwaay's stated different-party
\batchreceive condition and subjects it to the same controlled
removal/message-level/party-level comparison. This statement is intentionally
bounded: absence of a direct collision in the examined corpus is not an
exhaustive priority claim. The closest work supplies the protocol and condition
itself \cite{collins2024kwaay}, the composition-layer precedent
...
\cite{sattarzadeh2015injective}.

Against that background, two aspects of the present analysis have direct
support within the stated boundary. First, it gives a bounded symbolic
necessity analysis of an already stated \batchreceive condition through
relaxation and non-vacuous party-level restoration. Second, its message-level
control exhibits the protocol-specific separation in which scoped occurrence
injectivity holds while a same-party/different-message composition remains
reachable. Both claims concern the fixed two-slot admission abstraction and
