...
\cite[Sec.~4.1]{collins2024kwaayfull}. Writing the vector as
...
\cite[Sec.~5.1, Fig.~10]{collins2024kwaayfull}.
...
different party \cite[Sec.~5.1, p.~21, after Fig.~10]{collins2024kwaayfull}. This is a
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\linewidth}{@{}lXX@{}}
    \toprule
    Coordinate & What it distinguishes & Interpretation boundary \\
    \midrule
    Party $A$ & A modeled protocol principal & Not identified with a prekey
      or a key encoding \\
    Message $m$ & Sender-produced protocol content & Different messages need not
      have different originating parties \\
    Occurrence $\mathit{sid}$ & A sender-session/occurrence used in the analysis
      & Different occurrences need not have different parties \\
    Slot $i$ & A position within one batch & Different positions need not have
      different parties or messages \\
    Batch $B$ & One grouped receiver-side execution & Does not require global
      uniqueness of its participants \\
    Receiver state $r$ & Receiver state shared by the grouped execution & The
      identity or number of contributing parties is not determined by this coordinate \\
    \bottomrule
  \end{tabularx}
\end{table}

Table~\ref{tab:identity-coordinates} distinguishes conceptual roles, not a
claim that every combination of equalities is realizable in the prototype.
In Section~4, fresh $\mathit{sid}$ and $m$ are generated for every sender
occurrence. Under that assumption, legitimate origin tuples from different
parties also have different occurrence and message coordinates. The converse
does not follow: a single party may
