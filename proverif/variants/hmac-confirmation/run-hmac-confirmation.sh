...
MODEL="$ROOT_DIR/proverif/variants/hmac-confirmation/kwaay_core_hmac_confirmation.cpp.pv"
LOG_ROOT="$ROOT_DIR/logs/variants/hmac-confirmation/proverif"
...
{
  echo "K-Waay HMAC confirmation ProVerif variant summary"
  echo "model: $MODEL"
  echo "proverif: ${PROVERIF_CMD[*]}"
  echo "generated: $(date -u '+%Y-%m-%dT%H:%M:%SZ')"
  echo "TARGETS: ${ALL_TARGETS[*]}"
  echo "NOTE: this variant is explicit HMAC key confirmation over sid; it is not AEAD."
  echo
} > "$SUMMARY"
